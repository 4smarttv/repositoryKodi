...
class GUI( xbmcgui.WindowXMLDialog ):
    ...
    def onInit( self ):
        if self.searchstring == '':
            self._close()
        elif self.searchstring.isdigit(): # this if correct digit
            self.window_id = xbmcgui.getCurrentWindowDialogId()
            #xbmcgui.Window(self.window_id).setProperty('GlobalSearch.SearchString', self.searchstring)
            xbmcgui.Window(self.window_id).setProperty('karaokesearch.SearchString', self.searchstring)
            self._hide_controls()
    ...
    def _fetch_movies( self ):
        ...
        if count > 0:
            ...
            if xbmc.Player().isPlayingVideo():
              self._queue_video(path,listitem)
            else:
              #self._play_video(path)
              self._queue_play_video(path,listitem)
        else:
            xbmc.executebuiltin('Notification(Result not found!!!,Please! search again,5000)')
            self._close()

    def _fetch_moviesString( self ):
        listitems = []
        self.getControl( 191 ).setLabel( '[B]' + xbmc.getLocalizedString(342) + '[/B]' )
        count = 0
        json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "VideoLibrary.GetMovies", "params": {"properties": ["title", "streamdetails", "genre", "studio", "year", "tagline", "plot", "plotoutline", "runtime", "fanart", "thumbnail", "file", "trailer", "playcount", "rating", "mpaa", "director", "writer"], "sort": { "method": "label" }, "filter": { "or":[{"field":"title","operator":"contains","value":"%s"},{"field":"title","operator":"contains","value":"%s"}] }}, "id": 1}' % (self.searchstring, self.searchstring))
    ...
    # Tien add to get AdNumber from server
    def reqAdNumber( self, url):                    
        try:
            req = urllib2.Request(url)
            response = urllib2.urlopen(req)
            data = response.read()      ## this is all xml data file
            response.close()
            xbmc.log(msg='reqAdNumber: AdNumber,%s'%(data), level=xbmc.LOGDEBUG)
            return data
        except urllib2.URLError, e:
            addon_log('URL: '+url)
            if hasattr(e, 'code'):
                #addon_log('We failed with error code - %s.' % e.code)
                ##xbmc.executebuiltin("XBMC.Notification(LiveStreams,We failed get file! with error code - "+str(e.code)+",10000,"+icon+")")
                ## Tien Add
                return 4
            elif hasattr(e, 'reason'):  ## at this two exception can read data from local file
                ##xbmc.executebuiltin("XBMC.Notification(LiveStreams,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
                ## Tien Add
                return 4    
    # Tien add function to queue video into play list then play playlist
    def _queue_play_video( self, path, listitem):
        ...
        if playlistSize >=2:
          ...
          xbmc.Player().play(playlist,listitem,False,playlistSize+1)
        else:
          xbmc.executebuiltin('Notification(Loading Next Song, Please Wait!!!,5000)')
          xbmc.Player().play( playlist )
        dlog = xbmcgui.Dialog()
        if machine_id == 1:  # No Kara
          dlog.notification("Demo Karaoke 50 Songs","To Upgrade contact Authorized Dealer, or Call 1-800-558-9456",xbmcgui.NOTIFICATION_INFO, 30000)
        elif machine_id == 2: # Test No Kara
          dlog.notification("Test Demo Karaoke 50 Songs","To Upgrade contact Authorized Dealer, or Call 1-800-558-9456",xbmcgui.NOTIFICATION_INFO, 30000)
        elif machine_id == 3: # Kara
          dlog.notification("Karaoke Full 10000 songs","Order at www.4SmartTvBox.com or Call 1-800-558-9456",xbmcgui.NOTIFICATION_INFO, 30000)
        elif machine_id == 4: # Test Kara
          dlog.notification("Test Karaoke 10000 Songs","Order at www.4SmartTvBox.com or Call 1-800-558-9456",xbmcgui.NOTIFICATION_INFO, 30000)
        elif machine_id == 5: # Time Promote Kara
          dlog.notification("Promotion Karaoke Model","Order at www.4SmartTvBox.com or Call 1-800-558-9456",xbmcgui.NOTIFICATION_INFO, 30000)
        elif machine_id == 6: # Other
          dlog.notification("Other Model","Order at www.4SmartTvBox.com or Call 1-800-558-9456",xbmcgui.NOTIFICATION_INFO, 30000)
        elif machine_id == 7: # Other1
          dlog.notification("Other 1 Model","Order at www.4SmartTvBox.com or Call 1-800-558-9456",xbmcgui.NOTIFICATION_INFO, 30000)
        elif machine_id == 8: # Development
          dlog.notification("Development Model","Order at www.4SmartTvBox.com or Call 1-800-558-9456",xbmcgui.NOTIFICATION_INFO, 30000)
        while(not xbmc.abortRequested):
          xbmc.sleep(10000)
 
    # Tien add this queue function    
    def _queue_video_adv( self, path, listitem):
        self._close()
        playlist = xbmc.PlayList(xbmc.PLAYLIST_VIDEO)
        # add random number advertisement
        url = 'http://www.4SmartTVBox.com/Video/AdInfor.xml' 
        totalNumberAdv = self.reqAdNumber(url)
        advNumber = random.randint(1,int(totalNumberAdv))
        advPath = 'http://www.4SmartTVBox.com/Video/adv'+str(advNumber)+'.mp4'
        xbmc.log(msg='Karaoke: _queue_play_video: Adverize Path,%s'%(advPath), level=xbmc.LOGDEBUG)
        listitemAdv =xbmcgui.ListItem ('')
        listitemAdv.setInfo('video', {'Title': '', 'Genre': 'Adv'})
        playlist.add(advPath,listitemAdv)
        #add this selected song to queue
        playlist.add(path, listitem)

    def _queue_video( self, path, listitem):
        self._close()
        playlist = xbmc.PlayList(xbmc.PLAYLIST_VIDEO)
        playlist.add(path, listitem)
...
class MyPlayer_adv(xbmc.Player):
    _isAbortRequest_ = False
    def __init__ (self):
    ...
    def onPlayBackStarted(self):
        ...
        if xbmc.Player().isPlayingVideo():
          ...
          #advertise = 'videoplayback'
          advertise = 'http://www.4SmartTVBox.com/Video/adv'
          if advertise in playingFile:
            xbmc.executebuiltin('Notification(Loading Next Song, Please Wait!!!,5000)')
            xbmc.Player().playnext()
            xbmc.log(msg='Play the Karaoke file now', level=xbmc.LOGDEBUG)

    def onPlayBackEnded(self):
        # Will be called when xbmc stops playing a file
        xbmc.log( "LED Status: Playback Ended, LED OFF" )
        xbmc.log(msg='XBMC Playback Ended', level=xbmc.LOGDEBUG)
        playlist = xbmc.PlayList(xbmc.PLAYLIST_VIDEO)
        playlistSize = playlist.size()
        xbmc.log(msg='onPlayBackEnded: playlistSize: ,%s'%(playlistSize), level=xbmc.LOGDEBUG)
        position = playlist.getposition()
        xbmc.log(msg='onPlayBackEnded: Position: ,%s'%(position), level=xbmc.LOGDEBUG)
        if position == -1:
    ...
    def onPlayBackStopped(self):
        ...
        xbmc.log( "LED Status: Playback Stopped call xbmc.abortRequested = true, LED OFF" )
        xbmc.abortRequested = True
